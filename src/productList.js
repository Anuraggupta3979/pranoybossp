const productData = [
  {
    name: "Handpainted Wooden boxes",
    category: "Wooden Craft",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F12_20211007_172021_0011.png?alt=media&token=2f7ca61d-dd4c-4412-98cf-3610cb879fe2",
    material: "Wood",

    description:
      "Exclusively handcrafted and hand-painted with chromatic hues and intricate motifs, this set of boxes portrays the opulence of traditional art. Perfect for storing, jewelleries, accessories or as a memory keepsake box, the beauty of these boxes enhances the aura, giving it a touch of royalty.",
    featured: 1,
    hot: 1,
  },
  {
    name: "Carved Mirror Frame",
    category: "Wooden Craft",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2FWipeOut36_13_2021_073633.650000_2.jpg?alt=media&token=f4f7f52a-295c-445c-88dc-8ee46393f424",

    description:
      "Hand-carved by talented artisans, this antique-looking mirror frame with carved acanthus leaves and a distressed look will add a vintage touch to your dressing area.",
    featured: 0,
  },
  {
    name: "Wooden carved mirror frame",
    category: "Wooden Craft",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2Fpng_20211013_193804_0000.png?alt=media&token=7979c697-3e52-49ec-8b05-525b25350107",
    description:
      "This full-length mirror with a floral wooden frame, carved in a distressed manner with intricate design and whitewashed, gives a vintage look to your space.",
    featured: 1,
    hot: 1,
  },
  {
    name: "Wooden Keepsake box",
    category: "Wooden Craft",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2Fquattrociocchi-flight-seriesfinal_1.jpg?alt=media&token=f81bd07c-8d82-47b4-bdee-782823694077",
    material: "Wood",
    description:
      "This handcrafted wooden box comes with a stylish and unique design. A perfect keepsake box to store your cherishable items, be it jewellery, crystals, or memorable photographs. Its vintage and rustic look makes it a great product for gifting purposes as well.",
    featured: 0,
  },
  {
    name: "Hand Painted Wooden drawer set",
    category: "Wooden Craft",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F4_20211007_172021_0003.png?alt=media&token=361bd7dc-0d59-46b6-9d6e-c785b17ccbf4",
    material: "Wood",
    description:
      "This Multiple drawer set fitted in a wooden frame is one of a kind. The hand painted vibrant hues combined with a vintage look and simple yet sumptuous designs will definitely beautify your room along with the purpose of storage of your special belongings.",
    featured: 0,
  },
  {
    name: "Mango-wood Ceramic Trinket boxes",
    category: "Wooden Craft",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F5_20211007_172021_0004.png?alt=media&token=8ad9b318-e035-44d0-a1c8-f190205e59b5",
    material: "Wood",
    Dimensions: "Dimensions: 210 x 95 x 80 mm Draw size approx 4.2 cm x 6.5 cm",

    description:
      "A fantastic jewellery/keepsake box with a wooden frame fitted with three ceramic drawers to beautify your collection. The white and blue hand-painted daisy designs give it a stunning look.",
    featured: 0,
  },
  {
    name: "Wooden Cheese Board",
    category: "Wooden Craft",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F10_20211007_172021_0009.png?alt=media&token=f3427726-d2f9-4e05-803c-ea923758f113",
    material: "Wood",

    description:
      "Impress your guests by these exclusive rustic wooden serving platters which are handcrafted with utmost care. This charcuterie plate is surely a great addition to your kitchen ware.",
    featured: 0,
  },
  {
    name: "Two-Tier Wooden Serving platter",
    category: "Wooden Craft",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F23_20211008_111455_0022.png?alt=media&token=8b129dda-6966-44fc-9ab6-56094d758805",
    material: "Wood",

    description:
      "Serve the delicious dishes in this classic serving platter. Handmade with utmost care, this wooden platter has a rustic look that takes you back to the old era, yet with a modern finish, that enhances the beauty of the table.",
    featured: 1,
    hot: 1,
  },
  {
    name: "Bell shaped Wind Chime",
    category: "Terracotta",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F24_20211008_111455_0023.png?alt=media&token=e8a0035b-cded-4419-9452-33036b71c201",

    description:
      "Hand-painted by artisans with vibrant hues,this wind chime tinkles with euphony.. The perfect arrangement of the terracotta bells and birds, the colorful beads, and the catchy design is perfect to adorn your balcony with beauty as well as melody.",
    featured: 1,
    hot: 1,
  },
  {
    name: "Hand-painted Bell hanging",
    category: "Terracotta",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2FWipeOut22_09_2021_102213.418000.jpg?alt=media&token=1dd269db-5680-45f8-a034-de2420b46903",

    description:
      "Beautifully hand-painted in Red and Green, this minimalistic yet exquisite piece will surely enhance the beauty of your walls/doors.",
    featured: 0,
  },
  {
    name: "Decorative Hand-painted Tealight Candles",
    category: "Terracotta",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F7_20211008_111455_0006.png?alt=media&token=6977c822-73a6-49d6-bf15-415c339666d8",

    description:
      "Made of clay and handpainted with vibrant hues, these candles come along with zari platters with the most famous Rajasthani gota work. This festive season, spread the fragrance of love and warmth with these cute candles.",
    featured: 0,
  },
  {
    name: "Traditional Earthen terracotta Diyas",
    category: "Terracotta",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F8_20211008_111455_0007.png?alt=media&token=b9cb3e89-349e-4dd1-a9cd-525090244627",

    description:
      "Clay Diyas have been a part of Indian culture and rituals for a very long, the virtue of which can never be replaced by lamps or lights. Revive the traditional pottery of Indian culture and bring to your home these handmade Diyas, spreading goodness and purity.",
    featured: 0,
  },
  {
    name: "Traditional Earthen decorative Diyas",
    category: "Terracotta",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F9_20211008_111455_0008.png?alt=media&token=fefc6222-c4da-47fe-a41b-8e5d084c08c2",

    description:
      "A Diya is of utmost importance during Diwali and many other occasions. Get your home these beautiful traditional diyas, crafted impeccably with terracotta to lighten up your homes with purity and drive away all the evil forces.",
    featured: 0,
  },
  {
    name: "Handpainted Terracotta Lanterns",
    category: "Terracotta",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F25_20211008_111455_0024.png?alt=media&token=55c24a82-adc2-4681-bf66-c7bad1f69569",

    description:
      "A perfect amalgamation of contemporary art and traditional craftsmanship by the local artisans. These multicoloured lanterns are all you need to lighten up your space in the most beautiful way.",
    featured: 1,
    hot: 1,
  },
  {
    name: "Handmade terracotta Planter",
    category: "Terracotta",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F10_20211008_111455_0009.png?alt=media&token=0ee18fe9-2680-42ce-b503-b0b735052178",

    Dimensions: "Small",

    description:
      "Enhance the beauty of your home with some plants in these elegant and matte-finished terracotta planters. These will surely beautify your environment and freshen up your soul.",
    featured: 0,
  },
  {
    name: "Handmade oval-shaped terracotta planter",
    category: "Terracotta",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F10_20211007_172021_0009.png?alt=media&token=f3427726-d2f9-4e05-803c-ea923758f113",

    Dimensions: "Small and Medium",

    description:
      "Baked at high temperatures, these planters are not only durable but also provide a classy look to your lush green plants.",
    featured: 0,
  },
  {
    name: "Handpainted Tulsi planter",
    category: "Terracotta",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F14_20211009_222025_0001%20(1).png?alt=media&token=ef18e6e6-0be7-4c62-9bb4-9f811e75e6fe",

    description:
      "“Tulsi” or the “Indian Basil” is considered as one of the sacred plants. Hand-Painted with utmost divinity, this beautiful terracotta planter enhances the beauty and significance of Tulsi in the courtyard",
    featured: 1,
    hot: 1,
  },
  {
    name: "Terracotta Planters",
    category: "Terracotta",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F15_20211009_222025_0002%20(1).png?alt=media&token=78979a9e-283b-47c2-b520-e593a78190f6",

    Dimensions:
      "these are mixture of all type of pots size vary from 6 inch to 12 inches diameters",

    description:
      "These neoclassical antique-looking planters enhance the beauty of the garden and its eye-catchy design, a piece of attraction.",
    featured: 1,
    hot: 1,
  },
  {
    name: "Handmade round Terracotta Urli Bowl",
    category: "Terracotta",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2Fterracotta-urli-round-about-with-pumpkin-orange-scales-500x500.jpg?alt=media&token=60f9d018-742e-4a1a-84ba-c53f649ae81e",

    Dimensions: "Medium",

    description:
      "Festivals around the corner? Decorate your home exteriors with this beautiful urli having a traditional look. Decorate your sweet home with this Urli along with some floating candles, lotus, and roses, and add charm to your surroundings.",
    featured: 0,
  },
  {
    name: "Floral Montage Metallic Wall Decor",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F12_20211008_111455_0011%20(1).png?alt=media&token=d23fd07d-a1ae-439b-97d3-66453421c1f1",
    material: "Iron",
    Dimensions: "43in w x 21 in h",

    description:
      "Made of iron wire, coated with resplendent gold colour, this ultimate piece of wall decor is a perfect combo of aestheticism and modernism. A piece that soothes your eyes.",
    featured: 0,
  },
  {
    name: "Macrame Mirror wall hanging",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F1_20211014_083919_0000.png?alt=media&token=64f92c5a-b5bc-4691-9d9e-85d4a7f0ffe8",

    description:
      "The round hand-knotted macrame frame of this mirror adds a boho touch to your interiors. High quality with a super stylish appearance, the best piece to decorate your living space.",
    featured: 0,
  },
  {
    name: "Cotton Double Bedsheets(with 2 pillow covers)",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2FWipeOut19_13_2021_051913.437000.jpg?alt=media&token=dd9daf1d-a2ce-40a6-9efa-ce195dca0fc2",

    description:
      "The beautiful pink patterns on the fabric portray the rich cultural artistic patterns of India. This beautiful bedsheet adds a dash of style and royalty to your bedroom setup, being cozy and comfortable at the same time.",
    featured: 0,
  },
  {
    name: "Floral Double Bedsheet (with 2 pillow covers)",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2FWipeOut19_13_2021_051913.437000.jpg?alt=media&token=436389d6-ce64-4b11-9d78-fe7be78f6f1f",

    description:
      "This floral printed bedsheet is an epitome of Block printing- a process of printing patterns by the means of engraved wooden blocks. This classic bedsheet adds minimalistic yet a classic look to your bedroom.",
    featured: 0,
  },
  {
    name: "Sanganeri Cotton Double Bedsheet",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2FWipeOut20_13_2021_052053.604000_2.jpg?alt=media&token=a2c4ca3f-cae2-44a7-8400-e98f70421c7a",

    description:
      "Sanganeri print style is a type of block printing technique that originated in the royal state of Rajasthan. This bedsheet(along with 2 pillow covers), made using the same technique consisting of intricate designs and patterns is a perfect amalgamation of luxuriousness and comfort that adds an ethnic touch to your bedroom.",
    featured: 0,
  },
  {
    name: "Macrame Table mats",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F6_20211009_214235_0005.png?alt=media&token=11b8e700-4798-4ac9-be41-7ebc7ca96842",

    description:
      "Cozy and elegant, these handmade mats are must-haves to compliment your table. Start your day with a cup of coffee on these mats that refresh your soul and soothes your eyes.",
    featured: 0,
  },
  {
    name: "Antique Hand-painted wooden tray",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F2_20211014_083919_0001.png?alt=media&token=80f9a273-385f-4ee0-aaf6-713db0972140",

    description:
      "This vintage wooden tray, handpainted with acrylic floral designs, and cracked paint gives it a french country look. This antique tray with minor nicks is something that every antiquarian person would love in his/her collection.",
    featured: 0,
  },
  {
    name: "Mason Jar wooden Sconce",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2Fimage.jpg?alt=media&token=94d33c11-eb78-4c88-8a31-72913dd34f07",

    description:
      "Lighten up your space with warmth and grace with these rustic mason jar candle holders. The burnt wooden texture adds a classical vibe to your wall.",
    featured: 0,
  },
  {
    name: "Cotton Bedsheet with Quilt",
    category: "Textile Treasures",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2FWipeOut13_13_2021_071315.291000.jpg?alt=media&token=609b1083-e7d9-407b-97eb-fec61132e802",
    material: "Cotton",
    Dimensions: "Double bed reversible quilt king size",

    description:
      "Enrich your bedroom with this comfy cotton bedsheet and quilt set. The perfectly printed designs on it feature a distinctive style that gives your room traditional yet a classic and elegant feel.",
    featured: 0,
  },
  {
    name: "Block Printed Cotton Bedsheet",
    category: "Textile Treasures",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F8_20211009_214235_0007.png?alt=media&token=384ae16c-d238-49fd-b311-55c7648b3a15",
    material: "Cotton",
    Dimensions: "Double bed sheet king size",

    description:
      "Block printing is an art that connects you with the rich tradition and history of handmade crafts. This hand block printed cotton bedsheet(along with 2 pillow covers) is not only cozy and comfortable, but also enhances the look of your room.",
    featured: 0,
  },
  {
    name: "Cotton Bedcover cum comforter",
    category: "Textile Treasures",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F7_20211009_214235_0006.png?alt=media&token=fe067a42-0600-4640-b6b5-523b6531f10c",

    Dimensions: "Double bed quilt standard king size",

    description:
      "The beautiful pink motifs on the fabric portray the rich cultural artistic patterns of India. This beautiful bedcover can be used as a comforter too. A perfect combo of comfort and elegance.",
    featured: 0,
  },
  {
    name: "Round Jute Planter Baskets",
    category: "Cane-Bamboo-Jute",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F3.jpg?alt=media&token=8294a566-c3b0-479a-b070-ba6bbcb2a1ce",
    material: "Jute",
    Dimensions: '6.75"DIAMETER x 6.75"HIGH',

    description:
      "Not only stylish and versatile, but these planter baskets are environmentally friendly too. A perfect amalgamation of contemporary living and traditional craftsmanship, these baskets are one of a kind. Its loose nature allows you to transform it to any shape or height, making it ideal for decorating your desk/interiors with your favourite plant.",
    featured: 0,
  },
  {
    name: "Stylish Kouna Bag",
    category: "Cane-Bamboo-Jute",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2FWipeOut14_13_2021_051450.398000.jpg?alt=media&token=d8afc0be-a9fc-4a43-b813-899feb90c636",
    material: "Bamboo",
    Dimensions: "Standard Size",

    description:
      "Handcrafted with immense love and care by skilled artisans, this simple yet elegant bag is a must-have to carry your belongings in the most dignified way.",
    featured: 0,
  },
  {
    name: "Ethnic Natural fibre handbags",
    category: "Cane-Bamboo-Jute",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2Fezgif.com-gif-maker.jpg?alt=media&token=e0714009-324e-471b-b88d-b270714bada1",
    material: "Natural Fiber",

    description:
      "These eco-friendly natural straw bags are super stylish and at the same time are durable and sturdy. The combination of the sophisticated look and simplistic nature of these bags is what makes them more enthralling.",
    featured: 0,
  },
  {
    name: "Banana Fibre handmade laundry basket",
    category: "Cane-Bamboo-Jute",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2Fproduct-500x500.jpeg?alt=media&token=671539e4-189b-4394-9d8c-4d5fb31b27e4",
    material: "Cane and Banana Fiber",
    Dimensions: "Height 20 inches width 15 inches length 20 inches",

    description:
      "Handwoven with utmost care and perfection, its minimalist look and ivory colour will assuredly complement your home decor, serving the purpose of storage as well. This exquisite laundry basket is durable and easy to carry.",
    featured: 0,
  },
  {
    name: "Multi purpose Wicker Basket Set",
    category: "Cane-Bamboo-Jute",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F10_20211009_220339_0000.png?alt=media&token=f86d613d-ec0d-463b-a8fc-d67066b2bd1b",

    description:
      "Handcrafted by talented craftsmen, this set includes 7 neutral coloured baskets: 1 laundry basket with lid, 1 fruit basket, 1 planter, 1 oval basket, 1 duck basket, 1 moon basket, and 1 flower basket. A perfect multipurpose set.",
    featured: 0,
  },
  {
    name: "Hanging bamboo lampshades",
    category: "Cane-Bamboo-Jute",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F1561361236_bl-b-7_1529741745.jpg?alt=media&token=751dab52-b120-49b8-8df2-8844bf821edb",
    material: "Bamboo",

    description:
      "Illuminate your room with these bamboo lamps, flawlessly handcrafted by the talented artisans, spreading the radiance and warmth of traditional arts.",
    featured: 0,
  },
  {
    name: "Symmetrical Bamboo Wall shelf",
    category: "Cane-Bamboo-Jute",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F11_20211009_220339_0001.png?alt=media&token=8f199f0c-4f2d-44ad-ac67-b93aebd7906a",
    material: "Bamboo",

    description:
      "This elegant bamboo wall shelf with a symmetrical frame is a perfect choice for displaying and storing your small treasures in the most aesthetic way. Handmade by the most diligent craftsmen, this piece enhances the decor of your wall.",
    featured: 1,
    hot: 1,
  },
  {
    name: "Straw Clutch Handbag with handle",
    category: "Cane-Bamboo-Jute",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F12_20211009_220339_0002.png?alt=media&token=625907ba-242b-49ef-8ded-90504377a6cb",
    material: "Straw",

    description:
      "Made of Rattan material and straw, this clutch bag is stylish and fashionable, being sturdy and durable at the same time. Its subtle neutral colour adds a sense of sophistication and class.",
    featured: 0,
  },
  {
    name: "Curved Metallic Candle Holder",
    category: "Festival Decor",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F1_20211007_172021_0000.png?alt=media&token=565633fe-dbbe-4a7d-8649-858749d5a4b3",
    material: "Black Metal",
    Dimensions:
      "Set of 2 Each measures 6L x 4.25W x 18.5H in. Made of black metal",

    description:
      "Lighten up your walls with this beautiful wall sconce. Made of black metal, this product is not only durable but also a great piece of decor.",
    featured: 1,
    hot: 1,
  },
  {
    name: "Nordic Abstract Metallic candle holder",
    category: "Festival Decor",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F2_20211007_172021_0001.png?alt=media&token=8e368bd4-841f-42a6-8d51-0c94cf2dd9db",
    material: "Metal",

    description:
      "The exclusive craftsmanship for this piece of abstract nordic candle holder helps you light up the space more gracefully. Accent your tables with this chic representation of culture.",
    featured: 0,
  },
  {
    name: "Rustic Candle Sconce",
    category: "Festival Decor",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F6_20211007_172021_0005.png?alt=media&token=e1357736-b6e2-47a1-81eb-877bb5a75ca7",
    material: "Wood",

    description:
      "Enlighten your cozy space with this vintage-looking elegant candle stand. Handcrafted with love, this wooden stand is perfect to give your walls an antique yet sophisticated look.",
    featured: 0,
  },
  {
    name: "Black Lantern Wall sconce",
    category: "Festival Decor",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2Flamp.webp?alt=media&token=f8bb2bde-9592-4692-9c36-c96feb923671",

    description:
      "This primitive-style lantern candle holder with a matte black finish is perfect to recreate the traditional style of lightning. The beautiful looking metallic lanterns have one of the glass panels that can open to fit in the candles. The pine board along with it adds a perfect countryside look to it.",
    featured: 0,
  },
  {
    name: "Handmade Triangle-shaped 3-D Wall decor",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F13_20211008_111455_0012%20(1)-imresizer.jpg?alt=media&token=a71896b2-90e9-4eed-973d-0a75b475e75e",

    description:
      "This Handcrafted wall decor is an ideal elegant piece for your room. Made with iron, with a subtle finish and triangular patterns, this chic style wall decor is perfect to embellish your interiors.",
    featured: 0,
  },
  {
    name: "3-D Ginkgo leaves metallic Wall clock decor",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2FWipeOut10_08_2021_111029.485000.jpg?alt=media&token=7ccaa44e-4e25-4201-85bb-47830650a495",

    description:
      'Time is a storm in which we all are lost". Manage your precious time with this eye-catchy clock cum wall decor. Its metallic finish and the metallic ginkgo leaves are perfect to embellish your aesthetic ambiance with absolute elegance.',
    featured: 0,
  },
  {
    name: "Traditional musician figurines Showpiece",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F15_20211008_111455_0014.png?alt=media&token=aed4f0fa-4d47-4d09-b68d-ce6a0ccb873b",

    description:
      "These metallic figurines(set of 3) depict the true essence of the traditional musicians. Rajasthani attire, multicolored and hand-painted, this decor piece is ideal for gifting purposes too.",
    featured: 0,
  },
  {
    name: "Multicoloured Tribal musician figurines",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F16_20211008_111455_0015.png?alt=media&token=2cc55202-cf57-4083-a8c3-377098c9716d",

    description:
      "Portraying the tribal art, these iron figurines(Set of 3) provide a rustic touch along with vibrant hues. It's the best choice to adorn your space with a touch of culture and modern art.",
    featured: 0,
  },
  {
    name: "Lotus-Shaped Metallic Wind Chime",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F8_20211007_172021_0007.png?alt=media&token=c3f6bf49-5058-4894-a768-bbf55a021d34",
    material: "Metal",

    description:
      "The lotus-shaped metal framework, traditional bells with bright pink beads, is what makes it a unique wind-chime that will jazz up your boring wall with musical tinkles.",
    featured: 0,
  },
  {
    name: "Metallic Car-face Wall decor",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2FMDE6184_primary-1100x1100_2.jpg?alt=media&token=c4f96c12-1bab-4b91-bcf3-574ee8bf5b9c",
    material: "Metal",
    Dimensions: "Dimensions:48.00in x 5.00in x 31.00in",

    description:
      "This car-face fall hanging would bring out your love for vintage cars. Handcrafted by diligent artisans, this piece would add a vintage and rustic touch to your decor.",
    featured: 0,
  },
  {
    name: "Rajasthani traditional Musician Wall Decor",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F17_20211008_111455_0016.png?alt=media&token=e80ee229-2f8c-4c84-b836-c9c5c2ee9ed4",

    description:
      "The chromatic hues, the traditional outlook, and the artistic brilliance of this decorative wall hanging piece adds grace and elegance to your room making it a space worth admiring.",
    featured: 0,
  },
  {
    name: "Ganesha Wall-hanging",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F19_20211008_111455_0018.png?alt=media&token=16cd5ede-b293-4525-b8ed-b020bd9779c5",

    description:
      "Drive away all your sorrows and problems by bringing this decorative wall hanging of Vighnaharta Lord Ganesha playing bansuri to your home. Impeccably handcrafted, this amazing piece of decor adorns your space along with Ganesha’s blessings.",
    featured: 1,
  },
  {
    name: "Radha Krishna decorative wall hanging",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2Fradha-krishna-on-swing-wall-hanging-AE40.jpg?alt=media&token=a8d022f4-425a-404f-b3d5-0592b211e06e",

    description:
      "Radha-Krishna, a symbol of divine love, has been beautifully handcrafted into this wall decor. Portraying the folklore, this alluring piece of decor will surely add elegance to your interiors.",
    featured: 0,
  },
  {
    name: "Metallic Musicians Wall decor cum table showpiece",
    category: "Metallic Wonders",
    image:
      "https://drive.google.com/file/d/1cT3lFgMQ6WnjZI_13TlmeMT5HnxOtkfu/view?usp=sharing",

    description:
      "This decorative piece is a perfect blend of traditionalism with modern art. The Metallic fusion and embossed paints will decorate your space with ultimate grace.",
    featured: 0,
  },
  {
    name: "Metallic Bike Wall Clock cum decor",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2Firon-avenger-bike-wall-art-1-727x800.jpg?alt=media&token=91f02a06-d8ee-47a8-b819-60e5170980fe",

    description:
      "This bike-shaped iron wall decor piece is certain to grab everyone’s attention. The Abstract Design of this piece will add a classic vibe to your wall and interiors.",
    featured: 0,
  },
  {
    name: "Crescent Moon Floral Wall-decor",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2FPhoto%20from%20Jyoti%20Pandey.jpg?alt=media&token=8e0b99fc-b8be-403c-8c7c-74e5e95e8922",

    description:
      "This exclusive metallic wall decor piece adds a feather to the cap of your beautiful interiors. The moon-shaped framework with floral designs on it spreads the radiance of love, charm, and creativity.",
    featured: 0,
  },
  {
    name: "Metallic Flower wall decor",
    category: "Metallic Wonders",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F9_20211007_172021_0008.png?alt=media&token=5f29044d-5ab1-4a55-9a87-883ab1f84b7d",

    description:
      "Inspired by the natural flowers-an epitome of beauty, this exclusive piece of wall art embellishes your walls in the most beautiful way. Its intricate design and symmetry is what make it a perfect piece worth gifting too.",
    featured: 0,
  },
  {
    name: "Wooden hand-Mirrors",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F7_20211007_172021_0006.png?alt=media&token=8863357b-8181-4209-9f4a-f9634a2af788",
    material: "Wood",

    description:
      "Hand-carved with utmost care, these wooden hand mirrors give an elegant and aesthetic look to your dressing area. They seem to be straight out of a fairytale due to their pastel hue and design.",
    featured: 0,
  },
  {
    name: "Wheelbarrow-shaped Decor stand",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F11_20211007_172021_0010.png?alt=media&token=67f15ad6-127e-4740-b402-bfde840afd34",

    description:
      "This 3-tier display stand is a perfect decor piece to accessorize the corner of your space with ultimate grace. You can decorate this according to your taste with contrasting artificial flowers.",
    featured: 1,
  },
  {
    name: "Macrame Fringe Tea coasters",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F11_20211007_172021_0010.png?alt=media&token=67f15ad6-127e-4740-b402-bfde840afd34",

    description:
      "Cozy and elegant, these handmade coasters are must-haves to compliment your table. Start your day with a cup of coffee kept on these beautiful coasters that refresh your soul and soothes your eyes.",
    featured: 0,
  },
  {
    name: "Macrame Magazine holder",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F2_20211009_214235_0001.png?alt=media&token=5c467025-c1e6-4e75-adfb-6833143c525b",

    description:
      "Organize your desk in the most beautiful way with this handcrafted macrame magazine holder that adds a tint of bohemian artistic design. Lightweight, durable, and sturdy, it is perfect for a side couch as well.",
    featured: 0,
  },
  {
    name: "Macrame Mandala Dream Catcher",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F3_20211009_214235_0002.png?alt=media&token=c8e03ab9-6907-46f3-b6eb-03995b8dac08",

    description:
      "Let your good dreams pass through, catching away all the nightmares and negative vibes by this handmade Dream Catcher. The intricate ’Mandala’ design has been handcrafted by the magnificent artisans with utmost perfection to embellish your bedroom with positive vibes.",
    featured: 1,
  },
  {
    name: "Macrame Cushion Cover",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F4_20211009_214235_0003.png?alt=media&token=b7e09711-cfc1-4875-9c9a-df92420683c8",

    description:
      "A perfect blend of Bohemian texture and modern designs, this neutral-coloured cushion cover is perfect for an aesthetic and minimalistic touch to your couch/bed.",
    featured: 0,
  },
  {
    name: "Handmade Macrame cushion cover",
    category: "Home Utilities",
    image:
      "https://firebasestorage.googleapis.com/v0/b/craftjet-a6e1b.appspot.com/o/products%2F5_20211009_214235_0004.png?alt=media&token=e967cf6e-3486-4049-a60f-7cfd81a99a54",

    description:
      "Handmade by talented artisans, the intricate design of this zipper cushion cover is a perfect amalgamation of bohemian texture and ethnic style. Knotted with diligence, this pillowcase will perfectly compliment your couch/bed.",
    featured: 0,
  },
];

export default productData;
